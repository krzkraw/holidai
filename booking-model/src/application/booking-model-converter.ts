...
import { BookingMatrix } from '../domain/booking-matrix';
...
export class BookingModelConverter {
  async csvFileToTsFile(
    inputCsvPath: string,
    outputTsPath: string,
    exportName = 'bookingMatrix',
  ): Promise<BookingMatrix> {
  ...
  async tsFileToCsvFile(
    ...
    const matrix = BookingMatrix.fromSnapshot(snapshot);
